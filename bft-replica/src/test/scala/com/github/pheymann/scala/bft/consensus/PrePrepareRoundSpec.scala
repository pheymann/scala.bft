...
import com.github.pheymann.scala.bft.storage.StorageAction.{IsPrePrepareStored, StorePrePrepare}
...
class PrePrepareRoundSpec extends ScalaBftSpec {
  ...
  val interpreter = (isStored: Boolean) => new (ServiceAction ~> Id) {
    def apply[A](action: ServiceAction[A]): Id[A] = action match {
      ...
      case IsPrePrepareStored(_)  => isStored
  ...
  "The pre-prepare round" should {
    ...
    """.stripMargin in {
      ...
      checkState(PrePrepareRound.processLeaderPrePrepare(request, state).foldMap(interpreter(false)), "pre-prepare")
    }
    ...
    "send a prepare message to all other replicas if it receives a valid pre-prepare and request from the leader" in {
      ...
      checkState(
        PrePrepareRound.processFollowerPrePrepare(message, delivery, state).foldMap(interpreter(false)),
        "pre-prepare"
      )
      ...
      checkState(
        PrePrepareRound.processFollowerPrePrepare(invalidMessage, delivery, invalidState).foldMap(interpreter(false)),
        "nothing"
      )

      // already stored message
      checkState(
        PrePrepareRound.processFollowerPrePrepare(message, delivery, invalidState).foldMap(interpreter(true)),
        "nothing"
      )
    }
  }
